    ...
    >
      <div className="bg-card rounded-xl sm:rounded-2xl shadow-sm border hover:shadow-xl transition-all duration-300 hover:-translate-y-1 overflow-hidden h-full flex flex-col mx-1 sm:mx-1.5">
        ...
        <div className="p-3 sm:p-4 flex-1 flex flex-col">
          <h3 className="font-bold text-foreground text-sm sm:text-base mb-0.5 line-clamp-1">{product.name}</h3>
          <p className="text-muted-foreground text-xs mb-1.5">{product.model}</p>
          <div className="mb-2">
            <StarRating rating={product.rating} reviews={product.reviews} size="default" />
          </div>
...
export default function FeatureProduct() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const step = firstCard ? firstCard.offsetWidth + CARD_GAP : el.scrollWidth / featuredProducts.length
